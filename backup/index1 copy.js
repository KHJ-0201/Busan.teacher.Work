...
function downloadSavedTimetable() {
    database.ref(`${currentClass}/timetableStorage`).once('value', snap => {
        const info = snap.val();
        if(info && info.data) {
            // 📍 엑셀에 출력될 공식 헤더 (업로드 양식과 100% 동일한 규격)
            const exportHeaders = ["일수", "날짜", "요일", "교시", "훈련시간", "교과목", "능력단위명", "세부교과", "훈련교사", "훈련장소"];
            
            const sortedData = info.data.map(row => {
                // 내부 압축 부품명을 원래 규격으로 매핑하여 조립
                return {
                    "일수": row["일수"] || "",
                    "날짜": row["날짜"] || "",
                    "요일": row["요일"] || "",
            ...
            const ws = XLSX.utils.json_to_sheet(sortedData, { header: exportHeaders });
...
// [보안 추가] 내부평가지 진입 검문소 (스마트키 확인)
function checkEvalPassword() {
    const modal = document.getElementById('evalPwModal');
    const input = document.getElementById('evalPwInput');
    
    input.value = ""; // 초기화
    modal.style.display = 'flex'; // 모달 팝업
    input.focus(); // 즉시 타이핑 가능하게 커서 이동

    // 💡 별도의 엔터 없이 7자리가 되면 바로 실행되는 로직
    input.oninput = function() {
        if (this.value.length === 7) {
            if (this.value === "1532178") {
                // 인증 성공 시 즉시 이동
                modal.style.display = 'none';
                location.href = '../평가지/평가지.html?class=' + currentClass;
            } else {
                // 인증 실패 시
...
function closeEvalPwModal() {
    document.getElementById('evalPwModal').style.display = 'none';
}
