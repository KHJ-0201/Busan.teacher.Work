...
document.getElementById('backLink').href = `index1.html?class=${currentClass}`;
...
function renderTable(list) {
        ...
        us.forEach((u, i) => {
            ...
            tr.innerHTML = `
                ...
                <td rowspan="${us.length}" class="subject-cell">
                    ...
                    <select class="editable-input c-subject-type" onchange="handleSubjectTypeChange(this, '${rowGroupId}'); updateSelectColor(this);" style="font-size:10px; color:#333; margin-top:5px; border:1px solid #ccc; border-radius:3px; padding:2px; width:90%;">
                ...
                <td>
                    ...
                    <div class="unit-type-container" style="display:${displayStyle};">
                        <button type="button" class="type-toggle-btn ${u.unitType === '필수능력단위' ? 'active-req' : ''}" onclick="setUnitType(this, '필수능력단위')">필수</button>
                        <button type="button" class="type-toggle-btn ${u.unitType === '선택능력단위' ? 'active-opt' : ''}" onclick="setUnitType(this, '선택능력단위')">선택</button>
                        <input type="hidden" class="c-unit-type-val" value="${u.unitType || ''}">
                    </div>
                    <select class="editable-input c-eval-method" 
    ...
    <option value="평가자체크리스트" ${u.evalMethod === '평가자체크리스트' ? 'selected' : ''}>평가자체크리스트</option>
                ...
                <td><button class="btn btn-red" onclick="this.parentElement.parentElement.remove()">삭제</button></td>
            `;
...
async function downloadCourseTableExcel() {
    database.ref(`${currentClass}/masterData`).once('value', async snap => {
        const d = snap.val();
        if(!d || !d.courses) return alert("데이터가 없습니다.");
        ...
        if (isUnitTypeMissing) {
            if (!confirm("필수/선택 정보 없이 다운하시겠습니까?")) {
                return; // 취소 시 중단
            }
        }
...
async function downloadEvalMethodExcel() {
    if (!confirm("평가일정은 각 능력단위의 마지막 일입니다. 한번 더 일정 확인 하세요. 다운로드 하시겠습니까?")) {
        return;
    }
    const masterSnap = await database.ref(`${currentClass}/masterData`).once('value');
    const timetableSnap = await database.ref(`${currentClass}/fullTimetable`).once('value');
    const d = masterSnap.val();
    const fullTimetable = timetableSnap.val();

    if(!d || !d.courses || !fullTimetable) return alert("데이터가 부족합니다.");
...
async function downloadPreEvaluationPlan() {
    ...
    if(!d || !d.courses || !fullTimetable) return alert("데이터가 부족합니다. 시간표와 교과 데이터를 확인해주세요.");
...
async function saveAllDataWithAlert() {
    try {
        ...
        let lastSubjectType = ""; // 위쪽 행의 타입을 기억할 변수

        rows.forEach(r => {
            const typeSelect = r.querySelector('.c-subject-type');
            // 💡 현재 행에 타입 선택기가 있으면 그 값을 기억하고, 없으면 위쪽 행의 값을 사용합니다.
            if (typeSelect && typeSelect.value !== "") {
                lastSubjectType = typeSelect.value;
            }
        ...
        if (hasNCS && missingUnitType) {
            if (confirm("필수능력단위 외 나머지는 '선택능력단위'로 일괄 지정하여 저장하시겠습니까?")) {
                lastSubjectType = ""; // 변수 초기화 후 다시 순회
                rows.forEach(r => {
                    const typeSelect = r.querySelector('.c-subject-type');
                    if (typeSelect && typeSelect.value !== "") lastSubjectType = typeSelect.value;
        ...
        await saveAllData(); 
        alert("✅ 모든 변경사항이 서버에 안전하게 저장되었습니다.");
...
// 📍 [정밀 수리] 화면의 드롭박스 선택값까지 정확히 훑어서 서버로 쏘는 로직
async function saveAllData() {
    const rows = document.querySelectorAll('#courseTableBody tr:not(.summary-row)');
    const list = [];
    const ncsCodePattern = /[0-9]{2,}/; // 숫자가 포함되어 있으면 NCS로 인식

    rows.forEach(r => {
        const u = r.querySelector('.c-unit-input'); 
        if(u) {
            // 1. 교과목 이름 찾기 (병합된 셀 대응)
    ...
    return database.ref(`${currentClass}/masterData/courses`).set(list);
}
