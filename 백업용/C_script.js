...
function renderIntegratedTable() {
    ...
    [...config.ncs, ...config.nonNcs].forEach(main => {
        main.subSubjects.forEach(sub => {
            if (sub.date) subjects.push({ id: sub.name + sub.date, name: sub.name, date: sub.date, mainTitle: main.title });
        });
    });
...
function showIndividualReport(subId, userName) {
    ...
    [...(config.ncs || []), ...(config.nonNcs || [])].forEach(m => {
        m.subSubjects.forEach(s => { if (s.name + s.date === subId) questions = s.questions; });
    });
        ...
        if (img.complete) {
            ...
            img.onload = img.onerror = () => {
                ...
                if (loadedCount === images.length) {
                    setTimeout(() => window.print(), 300); // 렌더링 시간 확보
                }
            };
        }
    });
    ...
    return `
        <div class="result-page-container" style="width:190mm; margin:0 auto; font-family:'Malgun Gothic'; background:#fff; overflow:visible;">
            ...
            <table style="width:100%; border-collapse:collapse; border:2px solid #000; table-layout:fixed;">
                ...
                <tr><td style="${labelStyle}">훈련생명</td><td style="border:1px solid #000; padding:6px; font-weight:bold; text-align:left; font-size:12px;">${data.userName}</td><td style="${labelStyle}">시행일자</td><td style="${contentStyle}">${data.examDate}</td></tr>
            </table>
