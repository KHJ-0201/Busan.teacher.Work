        ...
        // 구(분) 단위 저장값을 시간(H)으로 환산 (예: 240 → 4)
        function normalizeEvalTestTimeToHours(val) {
            if (val === '' || val === null || val === undefined) return '';
            const n = parseFloat(String(val).trim());
            if (isNaN(n) || n <= 0) return String(val).trim();
            if (n >= 60 && n % 60 === 0) return n / 60;
            return n;
        }

        // 서류·PDF용: 설정(H) → 분 단위 표시 (예: 4 → 240)
        function formatEvalTestTimeForDoc(val) {
            const hours = normalizeEvalTestTimeToHours(val);
        ...
        async function fetchMasterData() {
            try {
                ...
                const [attSnap, tempAttSnap] = await Promise.all([
                    classDbRef('dailyAttendance').once('value'),
                    classDbRef('tempDailyAttendance').once('value')
                ]);
                const realAttendance = attSnap.val() || {};
                const tempAttendance = tempAttSnap.val() || {};
                globalAttendanceData = typeof mergeDailyAttendanceWithTemp === 'function'
                    ? mergeDailyAttendanceWithTemp(realAttendance, tempAttendance)
                    : realAttendance;
        ...
        function shouldIncludeEvidenceInEvalDocPdf() {
            // NCS: 본평가 평가원장에만 근거사진(빈 양식) 포함 / 비NCS·소양: 모든 평가 모드에서 근거자료 포함
            return currentEvalMode === '본평가';
        }
...
function generateEvalRowHtml(name, type, score, diff, question, coreTask, isNonNCS = false) {
    ...
    return `
        <tr class="eval-main-row">
            ...
            <td>
                <input type="number" class="eval-input eval-item-score" placeholder="배점" value="${score || ''}" ${isNonNCS ? 'readonly style="background:#fdf2e9; font-weight:bold; color:#d35400;" title="자동 계산됩니다."' : 'title="출제난이도 선택 시 자동 계산 · 직접 입력도 가능"'}>
            </td>
            ${isNonNCS ? '' : `
            <td><select class="eval-input eval-item-diff" onchange="autoDistributeNcsItemScores()" title="선택 시 80점 기준 배점 자동 계산">
...
window.NCS_KNOWLEDGE_SCORE_TOTAL = 80;

window.computeScoresByDifficulty = function(diffs, totalPoints) {
    const N = diffs.length;
    if (N === 0) return [];
    const total = Number(totalPoints) || window.NCS_KNOWLEDGE_SCORE_TOTAL;
    const SCORE_STEP = 5;
    const MIN_SCORE = 5;
    ...
    const nM = counts['중'];
    const nE = counts['하'];
    const tierWeight = { '상': 3, '중': 2, '하': 1 };
    const weightedSum = nS * tierWeight['상'] + nM * tierWeight['중'] + nE * tierWeight['하'];
    const idealTier = {
        '상': (total * tierWeight['상']) / weightedSum,
        '중': (total * tierWeight['중']) / weightedSum,
        '하': (total * tierWeight['하']) / weightedSum
    };

    const tierScores = { '상': 0, '중': 0, '하': 0 };
    const activeTiers = ['상', '중', '하'].filter(t => counts[t] > 0);

    const rangeScores = (from, to) => {
        const arr = [];
        for (let s = from; s <= to; s += SCORE_STEP) arr.push(s);
        return arr;
    };

    const scoreSum = (sS, sM, sE) => nS * sS + nM * sM + nE * sE;

    const tierDistance = (sS, sM, sE) => {
        let dist = 0;
        if (nS > 0) dist += Math.abs(sS - idealTier['상']);
        if (nM > 0) dist += Math.abs(sM - idealTier['중']);
        if (nE > 0) dist += Math.abs(sE - idealTier['하']);
        return dist;
    };
    ...
    if (activeTiers.length === 1) {
        ...
        for (let s = MIN_SCORE; s <= total; s += SCORE_STEP) {
            const gap = Math.abs(c * s - total);
            if (gap < bestGap) {
                bestGap = gap;
                bestScore = s;
            }
        }
        tierScores[t] = bestScore;
    } else {
        let best = null;
        let bestSumGap = Infinity;
        let bestDist = Infinity;

        const eValues = nE > 0 ? rangeScores(MIN_SCORE, total) : [0];
        for (const sE of eValues) {
            const mMin = nM > 0 ? (nE > 0 ? sE + SCORE_STEP : MIN_SCORE) : 0;
            const mValues = nM > 0 ? rangeScores(mMin, total) : [0];
            for (const sM of mValues) {
                const sMin = nS > 0 ? (nM > 0 ? sM + SCORE_STEP : (nE > 0 ? sE + SCORE_STEP : MIN_SCORE)) : 0;
                const sValues = nS > 0 ? rangeScores(sMin, total) : [0];
                for (const sS of sValues) {
                    const sum = scoreSum(sS, sM, sE);
                    const sumGap = Math.abs(sum - total);
                    const dist = tierDistance(sS, sM, sE);
                    if (sumGap < bestSumGap || (sumGap === bestSumGap && dist < bestDist)) {
                        bestSumGap = sumGap;
                        bestDist = dist;
                        best = { '상': sS, '중': sM, '하': sE };
                    }
    ...
    return diffs.map(d => {
        if (Object.prototype.hasOwnProperty.call(tierScores, d) && counts[d] > 0) return tierScores[d];
        return tierScores['중'] || MIN_SCORE;
    });
};

window.autoDistributeNcsItemScores = function() {
    if (window.isCurrentNonNCS) return;
    ...
    const diffs = [];
    rows.forEach(tr => {
        const diffSelect = tr.querySelector('select.eval-item-diff');
        diffs.push(diffSelect ? diffSelect.value : '중');
    });

    const scores = window.computeScoresByDifficulty(diffs, window.NCS_KNOWLEDGE_SCORE_TOTAL);
    rows.forEach((tr, idx) => {
        const scoreInput = tr.querySelector('input.eval-item-score') || tr.querySelector('td input[type="number"]');
        if (scoreInput) scoreInput.value = scores[idx];
    });
...
window.removeEvalRow = async function(btn) {
    ...
    if (await appConfirm("이 항목을 삭제하시겠습니까?")) {
        ...
        async function selectUnit(clickedBtn, subject, code, name, evalMethod, hours, mode, endDate, calendarEvalDate) {
            ...
            // 💡 [신규 지능형 모터] 평가 시간(교시 → 시간 H) 및 평가 장소 실시간 추출기
            let autoEvalTime = "";
            let autoEvalPlace = "";
            ...
            if (calendarEvalDate && globalEvaluationDates[mode] && globalEvaluationDates[mode][calendarEvalDate]) {
                ...
                if (evalData.details && Array.isArray(evalData.details)) {
                    ...
                    if (matchedDetail) {
                        ...
                        // 💡 교시 데이터를 시간(H)으로 변환 (예: "1~4" -> 4교시 -> 4H)
                        ...
                        if (pCount > 0) autoEvalTime = pCount; // 1교시 = 1시간(H)
                    }
                }
            }
            ...
            const rawEvalTime = (savedData && savedData.evalTestTime) ? savedData.evalTestTime : autoEvalTime;
            document.getElementById('S2_3').value = normalizeEvalTestTimeToHours(rawEvalTime);
                ...
                try {
                    ...
                    if (mainPlanData && mainPlanData.items && mainPlanData.items.length > 0) {
                        savedData = {
                            ...
                            evalTestTime: normalizeEvalTestTimeToHours(savedData?.evalTestTime || mainPlanData.evalTestTime),
            ...
            const ncsScoreAutoHint = document.getElementById('ncsScoreAutoHint');
            if (ncsScoreAutoHint) {
                ncsScoreAutoHint.style.display = window.isCurrentNonNCS ? "none" : "block";
            }
...
// --------------------------------------------------------------------
        ...
        window.page4BaseEvalMode = '본평가';
        window.page4ForcedStudent = "";

        window.getSavedEvalModeForStudent = function(stu) {
            const mode = window.studentScoresData?.[stu]?.savedMode;
            return (mode === '추가평가' || mode === '재평가' || mode === '본평가') ? mode : '본평가';
        };

        window.getEffectiveEvalModeForStudent = function(stu) {
            // 본평가 화면에서는 학생별 저장 모드를 우선 적용
            if (window.page4BaseEvalMode === '본평가') return window.getSavedEvalModeForStudent(stu);
            // 추가/재평가 화면에서는 현재 화면 모드를 유지
            return window.page4BaseEvalMode || currentEvalMode || '본평가';
        };

        /** 최종결과표 상단 제목 — 추가/재평가면 (추가평가)능력단위별… 형식 */
        window.getFinalResultTitleText = function(mode) {
            const m = (mode === '추가평가' || mode === '재평가') ? mode : '본평가';
            const prefix = m !== '본평가' ? `(${m})` : '';
            if (window.isCurrentNonNCS) return `${prefix}내부평가 최종결과표`;
            return `${prefix}능력단위별 내부평가 최종결과표`;
        };

        /** 추가/재평가 피드백 기본 서두 */
        window.getSpecialEvalFeedbackLead = function(mode) {
            if (mode === '추가평가') return '본평가일 공가 불참자로 추가평가 실시함.';
            if (mode === '재평가') return '본평가일 불참자로 최종점수에 90% 반영함.';
            return '';
        };

        window.prependSpecialEvalFeedbackLead = function(bodyText, mode) {
            const lead = window.getSpecialEvalFeedbackLead(mode);
            if (!lead) return String(bodyText || '');
            const body = String(bodyText || '').trim();
            if (!body) return lead;
            // 이미 서두가 있으면 중복 방지
            if (body.replace(/\s+/g, '').startsWith(lead.replace(/\s+/g, ''))) return body;
            return `${lead} ${body}`;
        };

        window.applyFinalResultTitle = function(rootEl, mode) {
            const root = rootEl || document;
            const titleEls = root.querySelectorAll
                ? root.querySelectorAll('.result-title')
                : [];
            const text = window.getFinalResultTitleText(mode);
            titleEls.forEach(el => {
                if (el) el.innerText = text;
            });
        };

        window.getEvalDateByMode = function(mode) {
            if (mode === '추가평가') return (document.getElementById('S3_2')?.value || "").trim();
            if (mode === '재평가') return (document.getElementById('S3_3')?.value || "").trim();
            return (document.getElementById('S3_1')?.value || "").trim();
        };

        window.getPage4SaveButtonHtml = function(mode) {
            if (mode === '본평가') {
                return `<button class="btn-save-settings" style="background:#e74c3c; border-color:#c0392b; padding: 6px 10px; font-size: 11px; margin-right: 4px;" onclick="deleteAllFinalResults()">🗑️ 전체 초기화</button><button class="btn-save-settings" style="background:#2980b9; border-color:#2980b9; padding: 6px 10px; font-size: 11px;" onclick="saveAllFinalResults()">💾 전체 통합저장</button>`;
            }
            return `<button class="btn-save-settings" style="background:#e67e22; border-color:#d35400; padding: 6px 10px; font-size: 11px;" onclick="saveIndividualFinalResult()">💾 ${mode} 개별저장</button>`;
        };

        window.refreshPage4HeaderByStudent = function(stu) {
            const effectiveMode = window.getEffectiveEvalModeForStudent(stu);
            const dateEl = document.querySelector('.page4-student-list-date');
            if (dateEl) {
                const evalDate = window.getEvalDateByMode(effectiveMode) || "미정";
                dateEl.innerText = `(평가일: ${evalDate})`;
            }

            window.applyFinalResultTitle(document.getElementById('page4') || document, effectiveMode);

            const modeBtns = document.querySelectorAll('#page4 .page4-mode-btn');
            modeBtns.forEach(btn => {
                const isActive = btn.getAttribute('data-mode') === effectiveMode;
                btn.style.borderColor = isActive ? '#2980b9' : '#bdc3c7';
                btn.style.background = isActive ? '#3498db' : '#ecf0f1';
                btn.style.color = isActive ? 'white' : '#7f8c8d';
            });
            if (typeof window.refreshPage4ModeButtonOutline === 'function') {
                window.refreshPage4ModeButtonOutline();
            }

            const saveArea = document.getElementById('page4_save_btn_area');
            if (saveArea) saveArea.innerHTML = window.getPage4SaveButtonHtml(effectiveMode);
            if (typeof window.refreshSpecialModeStudentOutline === 'function') {
                window.refreshSpecialModeStudentOutline();
            }
        };
        window.refreshSpecialModeStudentOutline = function() {
            const isMainModeList = window.page4BaseEvalMode === '본평가';
            const allBtns = document.querySelectorAll('.student-select-btn');
            allBtns.forEach(btn => {
                const stu = btn.getAttribute('data-student') || btn.innerText.trim();
                const savedMode = window.studentScoresData?.[stu]?.savedMode || '본평가';
                const isSpecialSaved = savedMode === '추가평가' || savedMode === '재평가';

                if (isMainModeList && isSpecialSaved) {
                    if (savedMode === '추가평가') {
                        btn.style.outline = '3px solid #6c2a8c';
                        btn.style.boxShadow = '0 0 0 2px rgba(142,68,173,0.35), inset 0 -3px 0 rgba(0,0,0,0.18)';
                    } else {
                        btn.style.outline = '3px solid #b34700';
                        btn.style.boxShadow = '0 0 0 2px rgba(211,84,0,0.35), inset 0 -3px 0 rgba(0,0,0,0.18)';
                    }
        ...
        window.refreshPage4ModeButtonOutline = function() {
            const allModeBtns = document.querySelectorAll('#page4 .page4-mode-btn');
            if (allModeBtns.length === 0) return;

            const scoreValues = Object.keys(window.studentScoresData || {})
                .filter(k => !k.startsWith('_'))
                .map(k => window.studentScoresData[k] || {});
            const hasAddModeStudent = scoreValues.some(v => v.savedMode === '추가평가');
            const hasReModeStudent = scoreValues.some(v => v.savedMode === '재평가');

            allModeBtns.forEach(btn => {
                const mode = btn.getAttribute('data-mode');
                if (mode === '추가평가' && hasAddModeStudent) {
                    btn.style.outline = '3px solid #8e44ad';
                    btn.style.outlineOffset = '1px';
                } else if (mode === '재평가' && hasReModeStudent) {
                    btn.style.outline = '3px solid #d35400';
                    btn.style.outlineOffset = '1px';
                } else {
                    btn.style.outline = '';
                    btn.style.outlineOffset = '';
        ...
        window.flashPage4ModeButton = function(mode) {
            const btn = document.querySelector(`#page4 .page4-mode-btn[data-mode="${mode}"]`);
            if (!btn) return;

            // 자동 전환 시 모드 인지를 높이기 위한 짧은 강조 이펙트
            btn.style.transition = 'transform 0.18s ease, box-shadow 0.18s ease, filter 0.18s ease';
            btn.style.transform = 'translateY(-1px) scale(1.06)';
            btn.style.filter = 'brightness(1.08) saturate(1.2)';
            btn.style.boxShadow = '0 0 0 3px rgba(52,152,219,0.32), 0 0 14px rgba(52,152,219,0.45), inset 0 -3px 0 rgba(0,0,0,0.18)';
        ...
        async function executeSave() {
            ...
            const saveData = {
                ...
                evalTestTime: normalizeEvalTestTimeToHours(document.getElementById('S2_3').value), 
        ...
        async function buildEvaluationDocs() {
            ...
            const evalTestTime = formatEvalTestTimeForDoc(document.getElementById('S2_3').value || ""); 
                ...
                if (stdData && stdData.elements && stdData.elements.length > 0) {
                    ...
                    stdData.elements.forEach((el, index) => {
                        ...
                        html += `<tr class="eval-main-row">
                                ...
                                <td><input type="number" class="eval-input eval-item-score" placeholder="배점 (예: 20)" title="출제난이도 선택 시 자동 계산 · 직접 입력도 가능"></td>
                                <td><select class="eval-input eval-item-diff" onchange="autoDistributeNcsItemScores()" title="선택 시 80점 기준 배점 자동 계산"><option value="상">상</option><option value="중" selected>중</option><option value="하">하</option></select></td>
                            </tr>
                            <tr class="eval-question-row">
        ...
        window.extractTraineeHeaders = function(html) {
            let defaults = ["① 측정값", "② 규정값", "③ 판 정", "④ 정비 및 조치사항"];
       ...
       // 💡 [신규 엔진] 교사 평가표 높이 자동 압축기 (Total Height 방어)
        ...
        const EVAL_ITEM_GAP_LINE_PX = 18;

        window.renderEvalItemGapZone = function(afterIdx, extraLines, gapSize) {
            const basePx = parseInt(String(gapSize), 10) || 20;
            const extra = Math.max(0, Number(extraLines) || 0);
            const totalH = basePx + extra * EVAL_ITEM_GAP_LINE_PX;
            const hintText = extra > 0
                ? `↕ 항목 간격 +${extra}줄 (Enter ↵ / Backspace)`
                : `↕ 항목 간격 기본 (Enter ↵ / Backspace)`;
            return `
                <div class="eval-item-gap-zone"
                     data-after-idx="${afterIdx}"
                     data-base-gap="${basePx}"
                     data-extra-lines="${extra}"
                     tabindex="0"
                     style="height: ${totalH}px;"
                     title="클릭 후 Enter: 한 줄 늘리기 / Backspace: 한 줄 줄이기">
                    <div class="no-print eval-item-gap-hint">${hintText}</div>
                </div>
            `;
        };

        window.applyEvalItemGapToZone = function(zone, extraLines) {
            if (!zone) return;
            const basePx = parseInt(zone.getAttribute('data-base-gap'), 10) || 20;
            const extra = Math.max(0, Number(extraLines) || 0);
            zone.setAttribute('data-extra-lines', String(extra));
            zone.style.height = (basePx + extra * EVAL_ITEM_GAP_LINE_PX) + 'px';
            const hint = zone.querySelector('.eval-item-gap-hint');
            if (hint) {
        ...
        window.adjustEvalItemGap = async function(zone, delta) {
            if (!zone) return;
            const afterIdx = zone.getAttribute('data-after-idx');
            if (afterIdx === null || afterIdx === '') return;
            let extra = parseInt(zone.getAttribute('data-extra-lines') || '0', 10) + delta;
            extra = Math.max(0, Math.min(30, extra));
            window.applyEvalItemGapToZone(zone, extra);
            if (!window.evalPaperItemGapLines) window.evalPaperItemGapLines = {};
            window.evalPaperItemGapLines[afterIdx] = extra;
            if (!currentDbKey) return;
            try {
                await classDbRef(`evalPlans/${currentEvalMode}/${currentDbKey}/itemGapLines/${afterIdx}`).set(extra);
            } catch (e) {
                console.error('항목 간격 저장 오류:', e);
            }
        };

        window.getEvalItemGapLines = function(savedGapLines) {
            const merged = Object.assign({}, savedGapLines || {}, window.evalPaperItemGapLines || {});
            const normalized = {};
            Object.keys(merged).forEach(k => {
                normalized[k] = Math.max(0, Number(merged[k]) || 0);
            });
            return normalized;
        };
        ...
        async function buildEvaluationPaper() {
            ...
            let savedItems = [];
            let savedItemGapLines = {};
            try {
                const snap = await classDbRef(`evalPlans/${currentEvalMode}/${currentDbKey}`).once('value');
                const savedData = snap.val();
                if (savedData) {
                    if (savedData.items) savedItems = savedData.items;
                    if (savedData.itemGapLines) savedItemGapLines = savedData.itemGapLines;
                }
            } catch (e) { console.error("DB 로드 에러:", e); }
            const itemGapLines = window.getEvalItemGapLines(savedItemGapLines);
            window.evalPaperItemGapLines = Object.assign({}, itemGapLines);
    ...
    <table class="ep-top-table" style="margin-bottom: 30px;">
                    ...
                    if (item.type === '훈련생작성') {
                        ...
                        let defaultTable = `
                            <table id="${tId}" class="ep-table trainee-table-target" style="margin-bottom: 5px; table-layout: fixed; width: 100%; word-break: break-all;">
                                ...
                                <tr>
                                    <th class="editable-cell">① 측정값</th>
                                    <th class="editable-cell">② 규정값</th>
        ...
        <td class="editable-cell" style="line-height:1.6; text-align:center;">
                        ...
                        tableHtml = `
                            ...
                            <div class="no-print" style="display:flex; gap:4px; margin-bottom:0; align-items:center; background:#edf7ff; padding:6px; border-radius:6px; border:1px solid #9dccf3;">
                        ...
                        tableHtml = `
                            ...
                            <div class="no-print" style="display:flex; gap:4px; margin-bottom:0; align-items:center; background:#ecf0f1; padding:6px; border-radius:6px; border:1px solid #bdc3c7;">
                    ...
                    const gapExtra = itemGapLines[idx] ?? itemGapLines[String(idx)] ?? 0;
                    const gapZoneHtml = window.renderEvalItemGapZone(idx, gapExtra, gapSize);
                    if (idx < frontLimit) frontHtml += tableHtml + gapZoneHtml;
                    else backHtml += tableHtml + gapZoneHtml;
...
            } catch (e) { console.warn("DB 로드 에러:", e); }
            ...
            let smallModeSwitchHtml = `
                <div style="display: flex; gap: 4px; margin-bottom: 12px; background: #f8f9fa; padding: 4px; border-radius: 6px; border: 1px solid #e1e4e8;">
                    <button class="page4-mode-btn" data-mode="본평가" style="flex:1; padding:6px 0; font-size:11px; font-weight:bold; border-radius:4px; border:1px solid ${currentEvalMode === '본평가' ? '#2980b9' : '#bdc3c7'}; background:${currentEvalMode === '본평가' ? '#3498db' : '#ecf0f1'}; color:${currentEvalMode === '본평가' ? 'white' : '#7f8c8d'}; cursor:pointer;" onclick="switchEvalType('본평가')">📌본평가</button>
                    <button class="page4-mode-btn" data-mode="추가평가" style="flex:1; padding:6px 0; font-size:11px; font-weight:bold; border-radius:4px; border:1px solid ${currentEvalMode === '추가평가' ? '#2980b9' : '#bdc3c7'}; background:${currentEvalMode === '추가평가' ? '#3498db' : '#ecf0f1'}; color:${currentEvalMode === '추가평가' ? 'white' : '#7f8c8d'}; cursor:pointer;" onclick="switchEvalType('추가평가')">➕추가</button>
                    <button class="page4-mode-btn" data-mode="재평가" style="flex:1; padding:6px 0; font-size:11px; font-weight:bold; border-radius:4px; border:1px solid ${currentEvalMode === '재평가' ? '#2980b9' : '#bdc3c7'}; background:${currentEvalMode === '재평가' ? '#3498db' : '#ecf0f1'}; color:${currentEvalMode === '재평가' ? 'white' : '#7f8c8d'}; cursor:pointer;" onclick="switchEvalType('재평가')">🔄재평가</button>
                </div>
            `;
            ...
            const evalModePrefix = (currentEvalMode === '추가평가' || currentEvalMode === '재평가') ? `(${currentEvalMode})` : "";
            const finalResultTitleText = (typeof window.getFinalResultTitleText === 'function')
                ? window.getFinalResultTitleText(currentEvalMode)
                : `${evalModePrefix}${window.isCurrentNonNCS ? '내부평가 최종결과표' : '능력단위별 내부평가 최종결과표'}`;
            ...
            if (studentList.length === 0 && currentEvalMode === '본평가') studentList = ["훈련생"]; 
            const initialSelectedStudent = (window.page4ForcedStudent && studentList.includes(window.page4ForcedStudent))
                ? window.page4ForcedStudent
                : studentList[0];
            ...
            let studentButtonsHtml = studentList.map((stu, index) => {
                let activeStyle = stu === initialSelectedStudent
                    ? "background:#2ecc71; color:white; border-color:#27ae60; box-shadow:inset 0 -3px 0 rgba(0,0,0,0.2);"
                ...
                if (targetDate && globalDropoutData[stu] && targetDate >= globalDropoutData[stu]) {
                    ...
                    // 💡 추가/재평가 화면에서는 해당 모드로 저장된 학생 점수만 표시 (나머지는 숨김)
                    const shouldShowInThisPage = (currentEvalMode === '본평가')
                        ? true
                        : (stuScoreObj.savedMode === currentEvalMode);
                    stuScore = (shouldShowInThisPage && stuScoreObj.totalScore !== undefined && stuScoreObj.totalScore !== "-")
                        ? stuScoreObj.totalScore + "점"
                        : "-점";
                }
            ...
            let saveBtnHtml = window.getPage4SaveButtonHtml(currentEvalMode);
            ...
            let studentTopBarHtml = `
                <div class="no-print page4-student-top-bar" style="position: sticky; top: 10px; z-index: 105; width: 100%; margin-bottom: 15px; background: white; padding: 10px 15px; border-radius: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); border: 2px solid #2ecc71; flex-shrink: 0; display: flex; align-items: center; box-sizing: border-box; justify-content: space-between;">
                    ...
                    <div class="page4-student-controls" style="margin-left: 15px; flex-shrink: 0; display: flex; align-items: center; gap: 8px; background: #f8f9fa; padding: 5px 10px; border-radius: 6px; border: 1px solid #e1e4e8;">
                        ...
                        <span id="page4_save_btn_area">${saveBtnHtml}</span>
                    </div>
                </div>
            `;
            ...
            if (window.isCurrentNonNCS) {
                ...
                html += `
                    <div class="result-title">${finalResultTitleText}</div>
            ...
            if (studentList.length > 0) {
                setTimeout(() => {
                    const initialStudent = initialSelectedStudent;
                    window.page4ForcedStudent = "";
                    const nameCell = document.getElementById('final_student_name_cell');
                    if (nameCell) nameCell.innerText = initialStudent;
                    if (initialStudent !== "훈련생") window.refreshPage4HeaderByStudent(initialStudent);
                    loadStudentState(initialStudent);
                    if (window.page4ModeFlashTarget) {
                        window.flashPage4ModeButton(window.page4ModeFlashTarget);
                        window.page4ModeFlashTarget = "";
                    }
                }, 50);
            } else {
                setTimeout(() => {
        ...
        window.selectStudent = async function(studentName, btnElement, skipModeSync = false) {
            saveCurrentStudentState(); // 💡 다른 학생으로 넘어가기 전 현재 작성중인 점수 메모리에 백업

            // 💡 본평가 화면에서 특수학생 클릭 시:
            // 학생 모드로 전환한 뒤, 해당 학생을 첫 선택 대상으로 강제 지정한다.
            if (!skipModeSync && window.page4BaseEvalMode === '본평가') {
                const desiredMode = window.getSavedEvalModeForStudent(studentName);
                if (desiredMode !== currentEvalMode) {
                    window.page4ForcedStudent = studentName;
                    window.page4ModeFlashTarget = desiredMode;
                    window.switchEvalType(desiredMode);
                    return;
                }
            }
            ...
            window.refreshPage4HeaderByStudent(studentName);
...
// 💡 [신규 모터 8] 미니 추적기 다이렉트 클릭 역산 엔진 (표를 클릭하면 점수가 됨)
        ...
        window.renderFeedbackZone = async function(forceCustomHtml = null) {
            ...
            let scoreMode = window.getEffectiveEvalModeForStudent(currentStu);
            let targetDateRaw = scoreMode === '본평가' ? dateMain : (scoreMode === '추가평가' ? dateAdd : dateRe);
            ...
            if (badge) {
                if (specialStatusText) {
                    ...
                    let currentMode = window.getEffectiveEvalModeForStudent(currentStu);
                    let modeHtml = "";
            ...
            if (zone) {
                const withLead = (text) => (typeof window.prependSpecialEvalFeedbackLead === 'function')
                    ? window.prependSpecialEvalFeedbackLead(text, scoreMode)
                    : text;

                if (forceCustomHtml !== null) {
                    // 저장된 커스텀 피드백에도 서두가 없으면 앞에 붙임 (텍스트만 추출해 판별)
                    const plain = String(forceCustomHtml).replace(/<br\s*\/?>/gi, ' ').replace(/<[^>]+>/g, '').trim();
                    const lead = (typeof window.getSpecialEvalFeedbackLead === 'function')
                        ? window.getSpecialEvalFeedbackLead(scoreMode)
                        : '';
                    const needsLead = !!(lead && plain && !plain.replace(/\s+/g, '').startsWith(lead.replace(/\s+/g, '')));
                    const customHtml = needsLead ? `${lead} ${forceCustomHtml}` : forceCustomHtml;
                    zone.innerHTML = `<div id="feedback_inner_text" ${editTag}>${customHtml}</div>`;
                } else {
                    if (allFilled && displayScore === 100) {
                        if (window.isJongWookMode) {
                            zone.innerHTML = `<div id="feedback_inner_text" ${editTag}>${withLead(`${unitNameStr} 작업을 전체적으로 우수하게 진행 하였으며, 과목이 끝났지만 평가내용을 지속적으로 숙지할 것을 독려함.`)}</div>`;
                        } else {
                            let levelStr = window.isCurrentNonNCS ? achievementLevel : achievementLevel + "단계";
                            zone.innerHTML = `<div id="feedback_inner_text" ${editTag}>${withLead(`총점 100점으로 ${unitNameStr} 수행 시 모든작업을 우수하게 진행하여 성취수준 ${levelStr}를 취득함.`)}</div>`;
                        }
                    } else {
                        let rawFeedbacks = Object.values(window.activeFeedbacks).filter(txt => txt && txt.trim() !== "");
                        if (rawFeedbacks.length === 0) {
                            // 점수는 채워졌지만 세부 피드백이 없는 추가/재평가 → 서두만 표시
                            const leadOnly = (typeof window.getSpecialEvalFeedbackLead === 'function')
                                ? window.getSpecialEvalFeedbackLead(scoreMode)
                                : '';
                            if (allFilled && leadOnly) {
                                zone.innerHTML = `<div id="feedback_inner_text" ${editTag}>${leadOnly}</div>`;
                            } else {
                            ...
                            zone.innerHTML = `<div id="feedback_inner_text" ${editTag}>` + withLead(finalFeedbacks.join(' ')) + `</div>`;
                        }
                    }
                }
            }
...
            const data = window.studentScoresData[stu];
            const isSpecialPage = (window.page4BaseEvalMode === '추가평가' || window.page4BaseEvalMode === '재평가');
            const isSameModeData = !!(data && data.savedMode && data.savedMode === window.page4BaseEvalMode);
            if (data && data.items && Object.keys(data.items).length > 0 && (!isSpecialPage || isSameModeData)) {
                window.currentEvalItems.forEach(item => {
...
        };
        ...
        window.saveAllFinalResults = async function() {
            ...
            if (!await appConfirm(`[전체 통합저장] 현재 화면에 보이는 전체 학생 점수를 저장하시겠습니까?\n(※ 본/추가/재 저장 모드(savedMode)를 그대로 보존하여 저장합니다.)`)) return;
        ...
        window.saveIndividualFinalResult = async function() {
            ...
            const activeMode = window.getEffectiveEvalModeForStudent(currentStu);
            // 선택된 학생에게만 현재 학생 기준 모드를 낙인합니다.
            window.studentScoresData[currentStu].savedMode = activeMode; 
            ...
            if (!await appConfirm(`[${activeMode}] '${currentStu}' 훈련생의 점수만 통합 DB에 업데이트하시겠습니까?\n(해당 학생의 최종 점수가 갱신됩니다.)`)) return;
            ...
            try {
                ...
                if (customCriteriaHtml) {
                    await classDbRef(`evalPlans/${activeMode}/${currentDbKey}`).update({ 
                        customCriteria: customCriteriaHtml,
                        customCriteriaSize: customCriteriaSize,
                        customCriteriaLineHeight: customCriteriaLineHeight
                    });
                ...
                await appAlert(`✅ '${currentStu}' 훈련생의 [${activeMode}] 점수 및 평가기준이 최종 갱신되었습니다!`);
...
                let stu = btn.innerText;
                let stuSavedMode = (window.studentScoresData[stu] && window.studentScoresData[stu].savedMode) ? window.studentScoresData[stu].savedMode : '본평가';
                let stuHasSpecialLockedScore = (stuSavedMode === '추가평가' || stuSavedMode === '재평가')
                    && window.studentScoresData[stu]
                    && window.studentScoresData[stu].totalScore !== "-"
                    && window.studentScoresData[stu].totalScore !== undefined;

                // 💡 본평가 일괄채점은 추가/재평가 확정자(점수 보유)를 절대 건드리지 않음
                if (currentEvalMode === '본평가' && stuHasSpecialLockedScore) {
                    skippedCount++;
                    return;
                }
...
                if (existingMode) window.studentScoresData[stu].savedMode = existingMode;
                else if (currentEvalMode === '본평가') window.studentScoresData[stu].savedMode = '본평가';
...
// =========================================================================
        ...
        function sanitizeBlankFinalResultPage(blankResultPage) {
            ...
            // 빈 양식도 현재 평가모드(추가/재평가) 제목 유지
            if (typeof window.applyFinalResultTitle === 'function') {
                window.applyFinalResultTitle(blankResultPage, currentEvalMode || '본평가');
            }
        }
        ...
        async function downloadPDF_1() {
            ...
            try {
                ...
                if (!window.isCurrentNonNCS) {
                    ...
                    // 💡 추가/재평가 평가원장 PDF에는 근거사진(빈 양식) 페이지 제외
                    if (shouldIncludeEvidenceInEvalDocPdf() && pages1.length > 3) targetElements.push(pages1[3].cloneNode(true)); // ④ 근거사진 (빈 양식)
                }
                ...
                // 💡 [비NCS] 순서 7: 근거자료(근거사진) — 본·추가·재평가 모두 포함
                if (window.isCurrentNonNCS) {
        ...
        async function downloadPDF_2() {
            ...
            try {
                ...
                for (let btn of studentBtns) {
                    ...
                    if (typeof window.refreshPage4HeaderByStudent === 'function') {
                        window.refreshPage4HeaderByStudent(stuName);
                    }
                    ...
                    if (page) {
                        ...
                        // PDF 클론에도 학생별 추가/재평가 제목 확실히 각인
                        if (typeof window.applyFinalResultTitle === 'function' && typeof window.getEffectiveEvalModeForStudent === 'function') {
                            window.applyFinalResultTitle(clone, window.getEffectiveEvalModeForStudent(stuName));
                        }
                ...
                if(studentBtns.length > 0) {
                    ...
                    if (typeof window.refreshPage4HeaderByStudent === 'function') {
                        window.refreshPage4HeaderByStudent(studentBtns[0].innerText);
                    }
                }
...
window.updateS5Panel = function(stdData) {
    ...
    equipments.sort((a, b) => String(a.name || '').localeCompare(String(b.name || ''), 'ko'));
...
// =========================================================================
        ...
        let traineeCellClickTimer = null;

        function updateTraineeEditorBanner(tId) {
        ...
        document.addEventListener('click', function(e) {
            ...
            if (!clickedTable || clickedTable.id !== window.currentActiveTableId) {
                clearTimeout(traineeCellClickTimer);
                traineeCellClickTimer = null;
            ...
            // 💡 2. 클릭한 칸 주황색 토글 (더블클릭과 겹치지 않도록 짧게 지연)
            clearTimeout(traineeCellClickTimer);
            traineeCellClickTimer = setTimeout(() => {
                clickedCell.classList.toggle('active-custom-cell');
                updateTraineeEditorBanner(clickedTable.id);
            }, 280);
        });
        ...
        document.addEventListener('dblclick', function(e) {
            ...
            let clickedCell = e.target.closest('.editable-cell');
            let clickedTable = e.target.closest('.trainee-table-target');
            if (!clickedCell || !clickedTable) return;
            if (clickedCell.isContentEditable) return;
            ...
            clickedCell.classList.add('active-custom-cell');
            updateTraineeEditorBanner(clickedTable.id);
            window.editCellText(clickedTable.id);
        });

        document.addEventListener('click', function(e) {
            const gapZone = e.target.closest('.eval-item-gap-zone');
            if (!gapZone) return;
        ...
        document.addEventListener('keydown', function(e) {
            ...
            if (e.key === 'Enter') {
                e.preventDefault();
                window.adjustEvalItemGap(gapZone, 1);
